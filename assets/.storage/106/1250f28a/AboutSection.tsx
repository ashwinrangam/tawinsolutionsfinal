...
export default function AboutSection() {
  ...
  const contentY = useTransform(scrollY, [0, 1000], [100, 0]);
  
  // Floating decorative elements
  const floatingElements = [
    { top: "10%", left: "5%", size: "150px", color: "rgba(212, 175, 55, 0.04)", delay: 0, transform: bgY1 },
    { top: "60%", right: "5%", size: "180px", color: "rgba(212, 175, 55, 0.03)", delay: 0.5, transform: bgY2 },
    { bottom: "20%", left: "15%", size: "120px", color: "rgba(212, 175, 55, 0.025)", delay: 0.2, transform: bgY1 },
  ];

  return (
    <section ref={sectionRef} id="about" className="relative section-padding overflow-hidden bg-gradient-to-b from-black to-background/95">
      {/* Background gradient with parallax effect */}
      <motion.div 
        style={{ y: bgY1 }}
        className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-gray-950 to-black opacity-80"
      ></motion.div>
      ...
      >
        {/* Vision Section - KEEP with parallax */}
        <motion.div
          ref={titleRef}
          initial={{ opacity: 0 }}
        ...
        {/* "SHAPING TOMORROW, TODAY" section - KEEP with enhanced parallax */}
        <ParallaxSection speed={0.4} className="mb-24 text-center">
          <motion.div
            className="relative"
            initial={{ opacity: 0 }}
            ...
            transition={{ duration: 0.8 }}
          >
            <h2 className="text-3xl md:text-4xl font-playfair font-bold text-center mb-8 gradient-heading relative z-10">
              SHAPING TOMORROW, TODAY
            </h2>
            {/* Subtle accent behind text */}
            <div className="absolute -inset-x-20 top-1/2 -translate-y-1/2 h-[1px] bg-gradient-to-r from-transparent via-gold/20 to-transparent"></div>
          </motion.div>
        </ParallaxSection>

        {/* Our Story - NEW with parallax */}
        <ParallaxSection speed={-0.2} className="mb-24">
          <motion.div
            ref={storyRef}
        ...
        {/* Final quote - KEEP with parallax */}
        <ParallaxSection speed={0.2} className="max-w-3xl mx-auto text-center mb-16">
          <motion.div 
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
        ...
        {/* Image grid with staggered parallax effects */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-16">
          <ParallaxSection speed={0.15}>
            <motion.div
              className="aspect-video rounded-xl overflow-hidden border border-gold/20"
              initial={{ opacity: 0, y: 20 }}
