...
export default function EnhancedContactSection() {
  ...
  async function onSubmit(data: ContactFormValues) {
    ...
    try {
      ...
      const serviceID = 'service_3bejs6p';
      const templateID = 'template_k67j8yd';
      const userID = 'PlZcSZg1nvFjbPdSI';
      const templateParams = {
        name: data.name,         // user's input
        email: data.email,       // user's input
        subject: data.subject,   // user's input
        message: data.message,   // user's input
      };
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 relative z-10">
                ...
                >
                  ...
                  {sendResult && (
                    <div className={`mt-2 text-center ${sendResult.includes('success') ? 'text-green-600' : 'text-red-600'}`}>{sendResult}</div>
                  )}
                </motion.div>
              </form>
            </Form>
